...
export class Search {
    ...
    public moe = async (params?: {query?: string, r18?: boolean, ugoira?: boolean, en?: boolean}) => {
        ...
        const kotoriToken = await this.api.request<{response: {access_token: string}}>("https://api.pixiv.moe/session")
            .then((r) => r.response.access_token)
        const response = await this.api.request<{response: {illusts: PixivIllust[]}}>(
            "https://api.pixiv.moe/v2/search",
            {word: params.query},
            {"x-kotori-token": kotoriToken}
        ).then((r) => r.response.illusts)
        return response
    }
}
