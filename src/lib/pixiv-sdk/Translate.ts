// @ts-nocheck
import {appendQuery, assertOk, requestWithTimeout} from "./adapters/Runtime"

/**
 * Translates search terms to japanese
 */
export default class Translate {
    private static translate = async (text: string, from: string, to: string) => {
        const url = appendQuery("https://api.mymemory.translated.net/get", {
            q: text,
            langpair: `${from}|${to}`
        })
        const response = await requestWithTimeout(url, undefined, {timeoutMs: 15000})
    ...
    public static translateTag = async (tag: string) => {
        ...
        try {
            return await Translate.translate(tag, "en", "ja")
        } catch {
            return tag
        }
    }
    ...
    public static translateTitle = async (title: string) => {
        try {
            return await Translate.translate(title, "ja", "en")
        } catch {
            return title
        }
    }
